function calculate(){
	...
	output.val('Ablösung in ' + payoffTime).set('payoff-time');
	output.val('WENN SIE EXTRA $500,00 PRO MONAT ZAHLEN').replace('$500.00', currencyFormat(payment)).set('payout-value');
	...
	if((totalRemainingMonths / 12) % 5 !== 0){
		chartLegendHtml += `<p class="result-text result-text--small">${(totalRemainingMonths / 12).toFixed(0)} Jahr</p>`;
	}
...
function getYearsMonths(months){
	let result = Math.floor(months / 12) + ' Jahre';
	if(months % 12 > 0){
		result += ' und ' + (months % 12) + ' Monate';
	}
	return result;
}
