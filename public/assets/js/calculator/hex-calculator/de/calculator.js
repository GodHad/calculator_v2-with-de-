function calculate(){
  ...
  const inputHex = (elementId, optional) => {
    ...
    if(!/^[0-9a-fA-F]+$/.test(value)) {
      input.error(elementId, `${elementId} muss eine natürliche Hex-Zahl sein.`);
    }
    return value;
  };
  switch($('[data-tab].tab--active').dataset.tab){
    ...
    case '1': 
      ...
      if(hexB == 0 && operation == 'divide') input.error('hex_b', 'Division durch null in hex_b');
      if(!input.valid()) return;
      ...
      if(operation == 'divide' && math.mod(decA,decB) != 0) {
        hexResult+= ` oder ${toHex(math.divide(decA-math.mod(decA,decB),decB))} Rest ${toHex(math.mod(decA,decB))}`;
        decResult+= ` oder ${math.divide(decA-math.mod(decA,decB),decB)} Rest ${math.mod(decA,decB)}`;
      }
      // 3. output
